...
Текущее состояние — **Phase 2 (Foundation)**: вход и сессии, роли и права, организации с иерархией, файлы, аудит, outbox и фоновые задачи, админ-панель. Спортсмены, турниры, жеребьёвка и судейство — следующие фазы ([план](docs/README.md#план)).
...
| `apps/worker` | Фоновые задачи: диспетчер outbox, письма, обслуживание БД (BullMQ) |
| `apps/web` | Веб-клиент Next.js: вход, регистрация, админ-панель; ru и en |
...
Сервис `migrate` применяет миграции и seed перед стартом `api` и `worker`. Все учётные записи seed вымышленные: см. вывод `migrate` или [packages/db/seed/data.ts](packages/db/seed/data.ts).
...
| `pnpm test:unit` | Unit-тесты: права и иерархия, ротация refresh token, правила организаций, сигнатуры файлов, шаблоны писем, словари |
| `pnpm test:integration` | Интеграционные тесты на реальных PostgreSQL и Redis: полный цикл входа, автотест безопасности маршрутов, аудит в транзакции, rate limit, организации, файлы |
