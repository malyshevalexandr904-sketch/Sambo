...
describe('create and approve', () => {
  it('a user without rights files a club for review; the federation admin approves it', async () => {
    ...
    expect(invalid.body.error).toMatchObject({
      ...
      details: { from: 'ACTIVE', allowed: ['SUSPENDED', 'ARCHIVED'] },
    });
  ...
  it('federation admin creates an ACTIVE child directly and edits it; the club manager of a sibling cannot', async () => {
    ...
    const cm = await login(
      t,
      (await createUser(t, { orgs: [{ organizationId: sibling, role: 'CLUB_MANAGER' }] })).email,
    );
  ...
  it('moves a subtree and keeps the closure table consistent', async () => {
    ...
    const sa = await login(t, (await createUser(t, { platform: ['SUPER_ADMIN'] })).email, { totp: true });
    await patch(sa, `/api/v1/organizations/${mid}`, { parentId: b }, 1).then((r) =>
      expect(r.status).toBe(200),
    );
    const rows = await t.admin.organizationClosure.findMany({
      where: { descendantId: leaf },
      orderBy: { depth: 'asc' },
    });
...
describe('members and invitations', () => {
  it('club manager invites a coach; only the invited email can accept', async () => {
    ...
    const invited = await post(cm, `/api/v1/organizations/${club}/members`, {
    ...
    expect(invited.body.data).toMatchObject({
      status: 'INVITED',
      roleCode: 'COACH',
      invitedEmail: 'coach@test.local',
    });
    const dup = await post(cm, `/api/v1/organizations/${club}/members`, {
      email: 'coach@test.local',
      roleCode: 'COACH',
    });
  ...
  it('cannot grant a role wider than your own; suspending a member removes rights immediately', async () => {
    ...
    const wider = await post(cm, `/api/v1/organizations/${club}/members`, {
      email: 'x@test.local',
      roleCode: 'ORGANIZER',
    });
    expect(wider.status).toBe(422);
    expect(wider.body.error.code).toBe('ROLE_EXCEEDS_GRANTOR');
    const platformRole = await post(cm, `/api/v1/organizations/${club}/members`, {
      email: 'x@test.local',
    ...
    const membership = await t.admin.organizationMembership.findFirstOrThrow({
      where: { userId: second.id },
    });
    const suspended = await patch(
      cm,
      `/api/v1/organizations/${club}/members/${membership.id}`,
      { status: 'SUSPENDED' },
      membership.version,
    );
...
describe('list and search', () => {
  it('searches by name ignoring case and ё, with cursor pagination', async () => {
    ...
    for (const name of ['Клуб «Ёрш»', 'Клуб «Ершов»', 'Спортшкола «Волна»']) {
      await post(sa, '/api/v1/organizations', {
        type: 'CLUB',
        name,
    ...
    const next = await sa.agent
      .get('/api/v1/organizations')
      .query({ q: 'ерш', limit: 1, cursor: res.body.page.nextCursor })
      .expect(200);
    expect(next.body.data).toHaveLength(1);
    expect(next.body.page.hasMore).toBe(false);
    expect([res.body.data[0].name, next.body.data[0].name].sort()).toEqual(
      ['Клуб «Ершов»', 'Клуб «Ёрш»'].sort(),
    );
