...
describe('rate limiting', () => {
  it('limits login attempts per email to 5 per minute with Retry-After', async () => {
    ...
    let last:
      { status: number; headers: Record<string, string>; body: { error: { code: string } } } | undefined;
    for (let i = 0; i < 6; i++) {
      last = await s.agent
        .post('/api/v1/auth/login')
        .set('x-csrf-token', s.csrf)
        .send({ email: u.email, password: 'wrong-password-x' });
  ...
  it('limits auth endpoints per IP to 10 per minute across different emails', async () => {
    ...
    for (let i = 0; i < 11; i++) {
      const res = await s.agent
        .post('/api/v1/auth/login')
        .set('x-csrf-token', s.csrf)
        .send({ email: `ip-${i}@test.local`, password: 'wrong-password-x' });
