...
const superAdmin = async (): Promise<Session> =>
  login(t, (await createUser(t, { platform: ['SUPER_ADMIN'] })).email, { totp: true });
...
describe('audit log', () => {
  it('is written inside the business transaction and rolled back with it', async () => {
    ...
    await RequestContextStore.run(ctx, () =>
      db.tx((tx) => audit.record(tx, { action: 'setting.updated', entityType: 'SystemSetting' })),
    );
  ...
  it('application role cannot update or delete audit records', async () => {
    ...
    try {
      ...
      await expect(appRole.$executeRaw`UPDATE audit_log SET action = 'tampered'`).rejects.toThrow(
        /permission denied/,
      );
  ...
  it('platform audit view filters by entity and hides raw IPs', async () => {
    ...
    const res = await sa.agent
      ...
      .query({ entityType: 'Organization', entityId: org.body.data.id })
      .expect(200);
...
describe('user administration', () => {
  it('blocks a user: all sessions end immediately; unblock restores access', async () => {
    ...
    await sa.agent
      .post(`/api/v1/admin/users/${victim.id}/unblock`)
      .set('x-csrf-token', sa.csrf)
      .send({ reason: 'Ошибка модератора' })
      .expect(200);
  ...
  it('platform roles require TOTP on the recipient; the last SUPER_ADMIN cannot be removed', async () => {
    ...
    expect(last.body.error).toMatchObject({
      ...
      details: { failed: ['last_super_admin'] },
    });
  });
...
describe('permission catalog', () => {
  it('database catalog equals packages/contracts (data migration is in sync)', async () => {
    ...
    const fromCode = ROLE_CODES.flatMap((r) =>
      Object.entries(ROLE_PERMISSIONS[r]).map(([p, m]) => `${r}:${p}:${m}`),
    ).sort();
...
describe('files', () => {
  ...
  it('uploads a logo through staging, verifies it and attaches it to an organization', async () => {
    ...
    const otherCm = await login(
      t,
      (await createUser(t, { orgs: [{ organizationId: other, role: 'CLUB_MANAGER' }] })).email,
    );
  ...
  it('rejects disguised content, wrong types, oversized files and purposes without a policy', async () => {
    ...
    const mismatch = await u.agent
      .post(`/api/v1/files/${ticket.body.data.fileId}/complete`)
      .set('x-csrf-token', u.csrf);
    expect(mismatch.body.error.code).toBe('FILE_CONTENT_MISMATCH');
    expect(
      (await t.admin.storedFile.findUniqueOrThrow({ where: { id: ticket.body.data.fileId } })).status,
    ).toBe('REJECTED');
...
describe('dictionaries and settings', () => {
  it('public dictionaries are cached and editable only with dictionary.manage', async () => {
    ...
    const regions = await anon.agent
    ...
    await sa.agent
      ...
      .expect(204);
    const again = await anon.agent
      .get('/api/v1/dictionaries/regions')
      .query({ countryCode: 'RU' })
      .expect(200);
    expect(again.body.data.length).toBe(84);
    const invalid = await sa.agent
      .put('/api/v1/admin/dictionaries/document-types/SCAN')
      .set('x-csrf-token', sa.csrf)
      .send({ nameRu: 'x' });
  ...
  it('settings are validated by schema; disabling self sign-up blocks registration', async () => {
    const sa = await superAdmin();
    const bad = await sa.agent
      .put('/api/v1/admin/settings/registration.selfSignupEnabled')
      .set('x-csrf-token', sa.csrf)
      .send({ value: 'yes' });
    expect(bad.body.error.code).toBe('VALIDATION_FAILED');
    const unknown = await sa.agent
      .put('/api/v1/admin/settings/unknown.key')
      .set('x-csrf-token', sa.csrf)
      .send({ value: true });
    ...
    expect(
      list.body.data.find((s: { key: string }) => s.key === 'registration.selfSignupEnabled'),
    ).toMatchObject({ value: false, isDefault: false });
