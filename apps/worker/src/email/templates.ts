...
const TEMPLATES: Record<Locale, Record<EmailTemplate, TemplateDef>> = {
  ru: {
    'auth.verify_email': {
      ...
      lines: ['Здравствуйте, {displayName}!', 'Чтобы завершить регистрацию, подтвердите адрес электронной почты. Ссылка действует 24 часа.'],
    ...
    'auth.password_reset': {
      ...
      lines: ['Мы получили запрос на смену пароля. Ссылка действует 1 час.', 'Если вы не запрашивали смену пароля, проигнорируйте письмо.'],
    ...
    'organization.invite': {
      ...
      lines: ['Вас пригласили в организацию «{organizationName}» с ролью «{roleName}».', 'Ссылка действует 7 дней. Войдите или зарегистрируйтесь с этим адресом email, чтобы принять приглашение.'],
  ...
  en: {
    'auth.verify_email': {
      ...
      lines: ['Hello, {displayName}!', 'Please confirm your email address to finish signing up. The link is valid for 24 hours.'],
      action: { label: 'Confirm email', param: 'verifyUrl' },
      required: ['verifyUrl', 'displayName'],
    },
    'auth.account_exists': {
      subject: 'You already have an account — SAMBO Digital',
      lines: ['Someone tried to sign up with this address, but an account already exists.', 'If it was you, sign in or reset your password. Otherwise, ignore this email.'],
      action: { label: 'Reset password', param: 'resetUrl' },
      required: ['resetUrl', 'loginUrl'],
    },
    'auth.password_reset': {
      subject: 'Password reset — SAMBO Digital',
      lines: ['We received a request to reset your password. The link is valid for 1 hour.', 'If you did not request it, ignore this email.'],
      action: { label: 'Set a new password', param: 'resetUrl' },
      required: ['resetUrl'],
    },
    'auth.password_changed': {
      subject: 'Password changed — SAMBO Digital',
      lines: ['Your password was changed and other sessions were signed out.', 'If this was not you, reset your password immediately and contact the administrator.'],
      action: { label: 'Reset password', param: 'resetUrl' },
      required: ['resetUrl'],
    },
    'organization.invite': {
      subject: 'Invitation to “{organizationName}” — SAMBO Digital',
      lines: ['You have been invited to “{organizationName}” as {roleName}.', 'The link is valid for 7 days. Sign in or sign up with this email address to accept.'],
...
export function renderEmail(template: EmailTemplate, locale: Locale, input: Params): RenderedEmail {
  ...
  if (missing.length > 0) throw new TemplateParamsError(`Missing params for ${template}: ${missing.join(', ')}`);
  const params: Params = { ...input, roleName: input.roleCode ? (ROLE_NAMES[locale][input.roleCode] ?? input.roleCode) : '' };
  ...
  const footer = locale === 'ru' ? 'Письмо отправлено автоматически, отвечать на него не нужно.' : 'This is an automated message, please do not reply.';
