...
Уточнения, найденные при реализации спортсменов, представителей, согласий, правил, категорий и документов. Согласованную архитектуру они не меняют.

### DATABASE.md

| Сущность | Изменение | Причина |
|---|---|---|
| `CoachMembership` | Новая таблица: тренер в организации периодом (`validFrom`, `validTo`), один открытый период на пару | Тренер клуба без аккаунта (или до приглашения) и история работы в клубах; «тренер спортсмена» проверяется по клубу спортсмена |
| `AthleteProfile` | `publicId` — 12 символов base58 (CHECK) | Публичный номер для будущих публичных страниц без раскрытия UUID |
| `AthleteMembership`, `AthleteCoach` | Partial unique: один открытый основной клуб и тренер; одно открытое членство в организации; CHECK периода | Переход между клубами закрывает прежний основной клуб днём раньше |
| `Guardian` | + `verificationBasis`, `endedAt` / `endedById` / `endReason` вместо удаления; partial unique активной связи | Подтверждение по документу или бумажному заявлению; история связей для аудита согласий |
| `ConsentTemplate` | unique `(kind, version, locale)`; триггер неизменяемости опубликованного текста; публикация новой редакции выводит прежнюю (`retiredAt`) | Согласие ссылается на ту редакцию, которую человек видел (ФЗ-152) |
| `Consent` | `competitionId` nullable; CHECK «скан → `documentId`» | Согласие на всё время и согласие на турнир |
| `RuleSetVersion` | Триггер неизменяемости `PUBLISHED` (разрешён только вывод в `RETIRED` без изменения содержимого); `checksum` = SHA-256 канонического JSON | ADR-09: страховка в БД поверх проверки в сервисе |
| `AgeGroup`, `WeightCategory` | Partial unique кода группы по владельцу `NULLS NOT DISTINCT` и границы веса; soft delete | Шаблоны платформы и организаций не конфликтуют |
| `Document` | Явные FK на спортсмена / заявку / организацию, CHECK «ровно один владелец»; + `reviewedById` / `reviewedAt` | Вместо полиморфной ссылки; кто проверил или отклонил |
| `ImportJob` | + `errorCode` (ошибка файла целиком), отчёт по строкам в `report` | Предпросмотр и итог импорта |
| `CompetitionMembership` | Пока нет таблицы `competition`, турнир «существует» через свой персонал | Проверка документов — турнирное право (`document.verify`); Phase 4 заменит источник области турнира |

### API.md

- **ETag — токен конкурентности, а не валидатор кэша** (уточнение 1.4). Производные поля ресурса (статус согласий, представители, `allowedActions`) меняются без смены версии, поэтому `If-None-Match: "v{n}"` не даёт `304`, а ответы API по умолчанию `Cache-Control: no-store` (персональные данные). Справочники — как раньше: `public, max-age=3600` и ETag содержимого.
- `GET /athletes` + `mine=true` — только спортсмены, у которых пользователь — действующий тренер. Спортсмены чужого клуба — `404` (не `403`): пользователь не вправе знать о них.
- `POST /athletes`: тренер клуба без `coachId` становится тренером сам; для этого у него должна быть запись «человек» (`coach_profile_required`). Дубли — `POSSIBLE_DUPLICATE` с кандидатами; создать всё равно можно, перечислив всех кандидатов и причину (`confirmNotDuplicate`).
- `Athlete` + `memberships`, `coaches` (периоды), `relation` (`SELF` / `GUARDIAN` для текущего пользователя); в `allowedActions` — `consent.give` / `consent.revoke`, когда пользователь вправе дать согласие электронно. Команды периодов и разрядов возвращают обновлённую карточку.
- **Представители:** добавлены `POST /athletes/{id}/guardians/{gId}/invite` (`{ email }` → `202`), `GET /me/athletes` (кабинет представителя и спортсмена), `POST /guardian-invites/accept` (`{ token }`; только владелец приглашённого подтверждённого email). Тот же родитель второго ребёнка — та же запись «человек».
- **Согласия:** бумажное согласие несовершеннолетнего подписывает подтверждённый представитель — `guardianId` в `ConsentCreate`; за совершеннолетнего — он сам. Повтор действующего согласия — `ALREADY_EXISTS`. Добавлены `GET /admin/consent-templates` и `PATCH /admin/consent-templates/{id}` (только черновик).
- **Тренеры:** `POST /coaches/{id}/end-membership` (`{ organizationId }`); `CoachCreate.confirmNotDuplicate`; `userId` — действующий участник организации с заполненными данными (`not_member`, `person_required`). **Судьи:** реестр общий для организаций — ведёт любой с `referee.manage` (федерация, платформа); `confirmNotDuplicate`.
- **Импорт:** фиксация синхронная — `200` с итоговым отчётом (не `202`); в шаблон добавлен `rankOrderRef`, заголовки можно писать по-русски. Импорт по матрице прав — руководитель клуба (`athlete.import`).
- **Правила:** добавлены `GET /rulesets/{id}` и `GET /rulesets/{id}/versions/{v}`; новая версия — из `parameters` или копией `basedOnVersion`. Ошибки параметров — `RULESET_PARAMETERS_INVALID` с путями полей.
- **Категории:** фильтр владельца `?owner=platform|{organizationId}`.
- **Документы:** проверка — только в контексте турнира (`competitionId` документа). Удаление — только загрузившим и только в `UPLOADED`. Скачивание по `fileId` подчиняется правам документа.
- `Idempotency-Key` обязателен на `POST /athletes/imports`; механизм общий (декоратор `@Idempotent()`), ответ хранится 24 часа.

### PERMISSIONS.md

- **Несколько областей ресурса.** Спортсмен виден и изменяем в любой организации своих текущих членств (клуб и спортшкола); право достаточно иметь в одной.
- **Политика `COACH_OWN`** зарегистрирована для `athlete.update`, `guardian.manage`, `consent.record`, `document.view`: действующий тренер спортсмена, работающий в его клубе.
- **Электронное согласие** — только подтверждённый представитель несовершеннолетнего или сам совершеннолетний спортсмен; у них же `document.view` / `document.upload` своих документов.
- **Импорт** — `CLUB_MANAGER` (матрица); в IMPLEMENTATION_PLAN импорт указан в кабинете тренера — оставлено по матрице, тренер добавляет спортсменов по одному.
